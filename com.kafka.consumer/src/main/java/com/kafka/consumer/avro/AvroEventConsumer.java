...
public class AvroEventConsumer
{
    ...
    public AvroEventConsumer(List<String> topics, int noOfConsumers,
        Properties props)
    {
	this.topics = topics;
	this.props = props;
	...
	executorService = Executors.newFixedThreadPool(noOfConsumers,
	    new ThreadFactory()
	    {
	        ...
	        {
		    return new Thread(r,
		        "EVENT-CONSUMER-THREAD-" + count.getAndIncrement());
	        }
	    });
    }
    ...
    {
	// Create consumers
	List<AvroEventDataConsumer> consumers = new ArrayList<AvroEventDataConsumer>(
	    noOfConsumers);
    ...
    private class AvroEventDataConsumer
        implements Callable<KafkaConsumer<String, GenericRecord>>
    {
	...
	public KafkaConsumer<String, GenericRecord> call()
	{
		...
		while (true)
		{
			...
			// records.
			System.out.printf("Processed by Consumer : "
			    + Thread.currentThread().getName()
			    + " => offset = %d, key = %s, value = %s, topic = %s, partition = %s\n",
			    record.offset(), record.key(),
			    record.value().get("name") + " "
			        + record.value().get("id") + " "
			        + record.value().get("age") + " "
			        + record.value().get("address") + " "
			        + record.value().get("salary"),

...
public class KafkaProducerImpl implements EventProducer
{
    ...
    private final BlockingQueue<ProducerRecord<String, Message>> queue = new ArrayBlockingQueue<ProducerRecord<String, Message>>(
        1024);
    ...
    ExecutorService myExecutorService = Executors
	    ...
	    {
	        return new Thread(r,
	            "Kafka-Producer-Thread-" + count.getAndIncrement());
	    }
        });
	...
	ProducerRecord<String, Message> record = new ProducerRecord<String, Message>(
	    topic, topicPartitionSelector.getNextPartition(), key, msg);
	queue.add(record);
    }
	...
	public void run()
	{
		...
		try
		{
		    ...
		    Future<RecordMetadata> result = myProducer.send(record,
		        new Callback()
		        {
			    @Override
